...
class Exporter {
    ...
    for (let device of devices) {
      try {
        ...
        try {
          ...
          //console.log(error);
          //console.log(`Error syncing interface frontend for VM ${device.virtual_machine_id}`);
        }

        await this.addUpdateIPs(device);
      } catch (error) {
        console.log(error);
        console.log(`Error syncing VM ${device.virtual_machine_id}`);
      }
    }
  ...
  async start() {
    console.log('Fetching VMs metadata...');
